...
// ⚡ BOLT OPTIMIZATION: Move regex to package level to avoid recompiling on every call.
var aliasValidationRegex = regexp.MustCompile("^[a-zA-Z0-9_-]+$")
...
func promptForAlias() (string, error) {
	...
	for {
		...
		if !aliasValidationRegex.MatchString(alias) {
